...
void HTTPSocket::do_read() {
  ...
  // Read a request
  http::async_read(
      getSocket(), _buffer, _req,
      boost::asio::bind_executor(
          getStrand(),
          std::bind(&HTTPSocket::on_read, ddddddd<HTTPSocket>(),
                    std::placeholders::_1, std::placeholders::_2)));
}

void HTTPSocket::on_read(boost::system::error_code ec,
...
void HTTPSocket::on_write(boost::system::error_code ec,
...
void HTTPSocket::handle_get() {
  ...
  body.open(_req.target().to_string().c_str(), boost::beast::file_mode::scan,
            ec);
...
void HTTPSocket::handle_post() { using namespace boost::beast; }
...
boost::beast::string_view HTTPSocket::mime_type(
...
HTTPSocket::bad_request(boost::beast::string_view why) {
  using namespace boost::beast;
...
boost::beast::http::response<boost::beast::http::string_body>
HTTPSocket::not_found(boost::beast::string_view target) {
  using namespace boost::beast;
...
boost::beast::http::response<boost::beast::http::string_body>
HTTPSocket::server_error(boost::beast::string_view what) {
  using namespace boost::beast;
