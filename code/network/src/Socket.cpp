...
void Socket::do_read() {
  async_read_head();
  _heartbeat.async_wait(boost::bind(&Socket::checkHeartBeat, shared_from_this(),
...
Boolean Socket::async_read_head() {
  ...
  async_read(_socket, buffer(buf, headSize), transfer_all(),
             _strand.wrap(boost::bind(&Socket::async_handle_read_head,
...
void Socket::async_handle_read_head(const boost::system::error_code& err,
                                    std::size_t len) {
  if (err && proError(err)) return;
...
void Socket::async_handle_read_body(const boost::system::error_code& err,
                                    std::size_t len) {
  if (err && proError(err)) return;
...
Boolean Socket::write(MsgData msg) {
  ...
  // 写入头部
  if (!writeImpl(msg.getHeadBuffer(), msg.getHeadSize())) return false;
...
Boolean Socket::asyncWrite(MsgData& msg) {
  ...
  msg.hostToNetwork();
  _strand.post(
      _strand.wrap(boost::bind(&Socket::do_write, shared_from_this(), msg)));
  return true;
}
...
void Socket::do_write(MsgData msg) {
  ...
  if (!write_in_progress) {
    _heartbeat.expires_from_now(boost::posix_time::seconds(_heartbeatTimeout));
    async_write(_socket,
                buffer(_writeMsg.front().getHeadBuffer(),
                       _writeMsg.front().getHeadSize()),
                _strand.wrap(boost::bind(&Socket::async_handle_write,
                                         shared_from_this(),
...
void Socket::async_handle_write(const boost::system::error_code& err, Int pos) {
  ...
  if (_writeMsg.front().step(pos, data, size)) {
    ...
    async_write(_socket, buffer(data, size),
                _strand.wrap(
                    boost::bind(&Socket::async_handle_write, shared_from_this(),
                                boost::asio::placeholders::error, pos + size)));
  } else {
    _writeMsg.pop_front();
    if (!_writeMsg.empty()) {
      _heartbeat.expires_from_now(
          boost::posix_time::seconds(_heartbeatTimeout));
      async_write(_socket,
                  buffer(_writeMsg.front().getHeadBuffer(),
                         _writeMsg.front().getHeadSize()),
                  _strand.wrap(boost::bind(
                      &Socket::async_handle_write, shared_from_this(),
                      boost::asio::placeholders::error, 0)));
    }
  }
}
...
void Socket::checkHeartBeat(const boost::system::error_code& err) {
  // TCP连接已终止或取消了心跳定时器
  if (_status == Status::S_Termination) return;

  // 真正的等待超时
  if (_heartbeat.expires_at() <= deadline_timer::traits_type::now()) {
    if (_conn->proHeartBeatTimeout()) {
      _conn->close();
      _heartbeat.expires_at(
          boost::posix_time::pos_infin);  // 定时器 设定为永不超时/不可用状态
    }
  }
