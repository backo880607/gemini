...
class HTTPSocket : public Socket
{
	...
	struct send_lambda
	{
		...
		void operator()(boost::beast::http::message<isRequest, Body, Fields>&& msg) const {
			...
			// Write the response
			boost::beast::http::async_write(self_.getSocket(), *sp,
				boost::asio::bind_executor(self_.getStrand(),
					std::bind(
						&HTTPSocket::on_write,
						self_.ddddddd<HTTPSocket>(),
						std::placeholders::_1,
						std::placeholders::_2,
						sp->need_eof())));
		}
...
private:
	...
	boost::beast::http::response<boost::beast::http::string_body> not_found(boost::beast::string_view target);
	boost::beast::http::response<boost::beast::http::string_body> server_error(boost::beast::string_view what);
