...
class CORE_API Any {
 public:
  ...
  class HolderClass : public Holder<T> {
   public:
    ...
    virtual PlaceHolder *clone() const {
      return new HolderClass(Holder<T>::_value);
    }
  ...
  template <typename T>
  PlaceHolder *create(const T &value) {
    return new HolderClass<T>(value);
  }
 ...
 public:
  ...
  Boolean isType() const {
    return getClass() == Class::forType<T>();
  }
  template <typename T1, typename T2>
  Boolean isType() const {
    return isType<T1>() || isType<T2>();
  }
  template <typename T1, typename T2, typename T3>
  Boolean isType() const {
    return isType<T1>() || isType<T2>() || isType<T3>();
  }
  template <typename T1, typename T2, typename T3, typename T4>
  Boolean isType() const {
    return isType<T1>() || isType<T2>() || isType<T3>() || isType<T4>();
  }
  ...
  template <typename T>
  typename ns_any::ObtainHolderType<
      typename ns_class::remove_cv<T>::type>::const_reference
  cast() const {
    return static_cast<Holder<typename ns_any::ObtainHolderType<
        typename ns_class::remove_cv<T>::type>::holder_type> *>(_holder)
        ->_value;
  }
