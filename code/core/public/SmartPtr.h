...
public:
	...
	friend inline pointer getImpl(const StorageNo& sp)
	{ return sp._pT; }
	friend inline const stored_type& getImplRef(const StorageNo& sp)
	{ return sp._pT; }
	friend inline stored_type& getImplRef(StorageNo& sp)
	{ return sp._pT; }
...
public:
	...
	friend inline pointer getImpl(const StorageSystem& sp)
	{ return sp._pT; }
	friend inline const stored_type& getImplRef(const StorageSystem& sp)
	{ return sp._pT; }
	friend inline stored_type& getImplRef(StorageSystem& sp)
	{ return sp._pT; }
...
class RefCounted
{
	...
	template <typename P1> friend class WeakCounted;
public:
	RefCounted() : _pCount(nullptr) {}
	RefCounted(const RefCounted& rhs) : _pCount(rhs._pCount) {}
	template <typename P1>
	RefCounted(const WeakCounted<P1>& rhs) : _pCount(reinterpret_cast<const WeakCounted<P1>&>(rhs).validCounter()) {}
	template <typename P1>
	RefCounted(const RefCounted<P1>& rhs) : _pCount(reinterpret_cast<const RefCounted&>(rhs)._pCount) {}
	...
	template <typename P1> friend class WeakCounted;
	template <typename P1> friend class RefCounted;
private:
...
public:
	...
	template <typename P1>
	WeakCounted(const RefCounted<P1>& rhs) : _pCount(reinterpret_cast<const WeakCounted&>(rhs)._pCount) {}
	template <typename P1>
	WeakCounted(const WeakCounted<P1>& rhs) : _pCount(reinterpret_cast<const WeakCounted&>(rhs)._pCount) {}
	...
	counter_type* validCounter() const {
		if (_pCount && _pCount->checkShareCount())
			return _pCount;
...
public:
	...
	template <typename T1,
		...
		SmartPtr(const SmartPtr<T1, OP1, MP1>& rhs)
		: MP(rhs), OP(rhs) {
	...
	~SmartPtr() {
		if (OP::release(getImpl(*reinterpret_cast<MP*>(this))))
			MP::destroy();
	}
	...
	inline Boolean valid() const { return OP::validImpl(getImpl(*this)) != nullptr; }
	...
	ID getID() const {
		return valid() ? getImpl(*this)->getID() : 0;
	}
