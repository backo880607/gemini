...
public:
	...
	~XMLNode() {}
	XMLNode& operator= (const DataNode& node) { DataNode::operator=(node); return *this; }
	...
	void setAttribute(const String& attr, const Char* val) { setAttribute(attr.c_str(), val); }
	void setAttribute(const Char* attr, const String& val) { setAttribute(attr, val.c_str()); }
...
private:
	String getXmlAttrName(const Char* attr) const {
		String attrName = "<xmlattr>."; attrName.append(attr);
		return attrName;
	}
};
...
public:
	...
	static void foreach(const Char* directory, std::function<void(XMLFile&)> fun);
	static void foreach_recursion(const Char* directory, std::function<void(XMLFile&)> fun);
private:
	void clear();
