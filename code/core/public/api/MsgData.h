...
class CORE_API MsgData {
 ...
 public:
  ...
  static const MSG_UINT32 s_maxBlock =
      64 * 1024;  ///< 最大的消息传输块大小，超过该长度的消息，分块传输
 ...
 public:
  ...
  EncodingType getEncoding() const {
    return (EncodingType)(getHead().dataEncode & 15);
  }
  /**
   * @brief 设置数据编码类型
   * @see EncodingType
  ...
  void setEncoding(EncodingType enType) {
    getHead().dataEncode =
        ((getHead().dataEncode >> 4) << 4) | (MSG_UINT8)enType;
  }
  ...
  CompressType getCompressType() const {
    return (CompressType)(getHead().cptEncryPri >> 6);
  }
  /**
   * @brief 设置数据压缩类型
   * @see CompressType
  ...
  void setCompressType(CompressType cpt) {
    getHead().cptEncryPri =
        (getHead().cptEncryPri & 63) | ((MSG_UINT8)cpt << 6);
  }
  ...
  void setEncryption(Encryption encry) {
    getHead().cptEncryPri = ((getHead().cptEncryPri >> 6) << 6) |
                            (((MSG_UINT8)encry << 4) | getPriority());
  }

  /**
   * @brief 获取消息优先级
   */
  MSG_UINT8 getPriority() const {
    return MSG_UINT8(getHead().cptEncryPri & 15);
  }
  /**
   * @brief 设置消息优先级
 ...
 private:
  ...
  MsgHead _head;  ///< 消息头
  std::shared_ptr<boost::asio::basic_streambuf<std::allocator<Char>>>
      _data;  ///< 数据
};
