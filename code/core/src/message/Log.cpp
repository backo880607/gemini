...
// A logger class that allows to intercept exceptions and supports severity
// level
class gemini_logger : public src::basic_composite_logger<
                          char, gemini_logger, src::single_thread_model,
                          src::features<src::severity<Logger::Level>,
                                        src::exception_handler> > {
  typedef typename gemini_logger::logger_base base_type;
  BOOST_LOG_FORWARD_LOGGER_MEMBERS(gemini_logger)
 public:
  explicit gemini_logger(severity_level level)
      : base_type(keywords::severity = level) {}
};

Logger::Logger(const char* file, int line, Level level /* = Level::LOG_INFO */)
    : _level(level), _logger(new gemini_logger(level)) {}
...
void Logger::init() {
  ...
  auto file_sink = logging::add_file_log(
      "sample.log", keywords::filter = _severity >= Level::INFO,
...
void Logger::setMinLevel(Level level) {
  boost::log::core::get()->set_filter(
      expr::attr<Level>("Severity").or_default(Level::INFO) >=
          level  // Write all records with "warning" severity or higher
      || expr::begins_with(
             expr::attr<std::string>("Tag").or_default(std::string()),
             "IMPORTANT"));  // ...or specifically tagged
}
