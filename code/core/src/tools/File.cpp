...
FilePath::FilePath(const FilePath& path) : _path(path._path) {}
...
String FilePath::string() const { return _path->string(); }
...
String FilePath::getName() const { return _path->filename().string(); }

String FilePath::getExtension() const { return _path->extension().string(); }
...
Boolean FilePath::isAbsolute() const { return _path->is_absolute(); }

Boolean FilePath::isRelative() const { return _path->is_relative(); }
...
FilePath FilePath::parent() const { return _path->parent_path(); }
...
FilePath FilePath::currentPath() {
  if (s_currentPath.empty()) {
    ...
    if (dll) {
      ...
      DWORD n =
          ::GetModuleFileNameA(dll, fileName.begin(), (DWORD)fileName.size());
...
void FilePath::currentPath(const Char* str) { s_currentPath = str; }
...
void File::setFilePath(const String& path) { _path.reset(path); }
...
Boolean File::isOpen() const { return _file.is_open(); }

void File::flush() { _file.flush(); }

File& File::write(const Char* str, std::streamsize count,
...
void File::read(Char* pBuf, Long size) { _file.read(pBuf, size); }

Boolean File::isEnd() const { return _file.eof(); }

File::pos_type File::getWritePos() { return _file.tellp(); }

void File::setWritePos(pos_type pos) { _file.seekp(pos); }

File::pos_type File::getReadPos() { return _file.tellg(); }

void File::setReadPos(pos_type pos) { _file.seekg(pos); }
