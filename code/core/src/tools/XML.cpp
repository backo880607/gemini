...
String XMLNode::getValue() const { return _pNode->get_value<String>(); }
...
Boolean XMLFile::open(const String& name,
                      File_Mode mode /* = File_Mode::NormalFile */) {
  using namespace boost::property_tree;
  clear();
  String xmlName = name;
...
Boolean XMLFile::write(const String& name,
                       File_Mode mode /* = File_Mode::NormalFile */) {
  String xmlName = name;
  if (!proXMLFile(xmlName, mode)) return false;
...
void XMLFile::foreach (const Char* directory,
...
void XMLFile::foreach_recursion(const Char* directory,
                                std::function<void(XMLFile&)> fun) {
  FilePath dirPath(directory);
  if (dirPath.valid()) {
