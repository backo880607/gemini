...
void PropagateHelper::loadConfig() {
  ...
  XMLFile::foreach_recursion(path.string().c_str(), [&](XMLFile& xmlFile) {
    ...
    THROW_IF(!propertyNode.valid(), Exception, u8"invalid property node: ",
             clsName)
    ...
    propertyNode.foreach ([&](XMLNode propertyNode) {
      ...
      THROW_IF(!funNode.valid(), Exception, u8"invalid function node: ",
               clsName)
      ...
      THROW_IF(!dependencesNode.valid(), Exception,
               u8"lost the dependence path: ", clsName)
      dependencesNode.foreach ([&](XMLNode depNode) {
        ...
        StringUtil::parse<String>(strDep, u8";", [&](const String& strField) {
          ...
          THROW_IF(fields.size() != 2, Exception, u8"invalid dependence path: ",
                   strField)
          EntityFactory* depFactory =
