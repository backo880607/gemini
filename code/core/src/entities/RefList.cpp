...
void RefList::add(EntityObject::SPtr entity) { base_type::push_back(entity); }

void RefList::remove(EntityObject::SPtr entity) { base_type::remove(entity); }

void RefList::remove() { base_type::clear(); }

class RefListIterator : public IList::Iterator {
  typedef std::list<EntityObject::SPtr> value_type;

 public:
  RefListIterator(const value_type::const_iterator& iter,
