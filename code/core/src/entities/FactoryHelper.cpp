...
void FactoryHelper::loadConfig() {
  ...
  XMLFile::foreach_recursion(path.string().c_str(), [&](XMLFile& xmlFile) {
    ...
    rootNode.foreach ([&](XMLNode relaNode) {
      ...
      THROW_IF(firstSign == 0, Exception, u8"invalid field name: ",
               firstSignName, u8" which bean is: ", clsName)
      ...
      THROW_IF(secondFactory == nullptr, Exception, u8"invalid bean name: ",
               secondFactoryName)
      String secondSignName = relaNode.getAttribute(u8"reverse");
      Int secondSign = secondFactory->getRelaSign(secondSignName);
      THROW_IF(secondSign == 0, Exception, u8"invalid field name: ",
               secondSignName, u8" which bean is: ", secondFactoryName)
      ...
      THROW_IF(firstData == nullptr, Exception,
               u8"entity factory relaData init failed.")
      ...
      EntityFactory::RefType type = typeName.empty()
                                        ? EntityFactory::RefType::T_List
                                        : EntityFactory::RefType::T_Set;
      switch (rela) {
      ...
      firstData->owner =
          StringUtil::icompare(ownerName.c_str(), u8"true") == 0 ? true : false;
