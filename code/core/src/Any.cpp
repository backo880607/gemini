...
class HolderEntityObject : public Any::Holder<SmartPtr<EntityObject>> {
 public:
  HolderEntityObject(const SmartPtr<EntityObject>& value)
      : Any::Holder<SmartPtr<EntityObject>>(value) {}
  ...
  virtual Boolean operator==(const Any& rhs) const {
    if (getClass() == rhs.getClass()) {
      return _value->getID() == rhs.cast<EntityObject>()->getID();
    }
    THROW(OperandException);
  }
  virtual Boolean operator<(const Any& rhs) const {
...
String Any::str() const { return *this ? _holder->str() : ""; }
...
String AnyAbstract::str() const { THROW(ClassCastException); }
