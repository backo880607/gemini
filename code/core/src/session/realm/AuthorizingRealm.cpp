...
AuthorizingRealm::~AuthorizingRealm()
{
	delete _permissionResolver;
}

Boolean AuthorizingRealm::isPermitted(const String& principal, const String& permission)
{
	std::shared_ptr<ObjectPermission> p = getPermissionResolver()->resolve(permission);
	std::shared_ptr<AuthorizationInfo> info = getAuthorizationInfo(principal);
	if (!p || !info) {
		return false;
	}

	const std::vector<std::shared_ptr<ObjectPermission>>& perms = info->getObjectPermissions();
...
std::shared_ptr<AuthorizationInfo> AuthorizingRealm::getAuthorizationInfo(const String& principal)
{
