...
Boolean Process::SharedMemory::open(Long id /* = 0 */) {
  ...
  try {
    if (id > 0) {
      ...
      _smem = smem_ptr(new shared_memory_object(
          create_only, strSharedMemName.c_str(), read_write));
      ...
      _smem = smem_ptr(new shared_memory_object(
          open_only, strSharedMemName.c_str(), read_write));
      _region = region_ptr(new mapped_region(*_smem.get(), read_write));
      void* addr = _region->get_address();
...
Process::Process(std::shared_ptr<ProcessImpl> impl) : _impl(impl) {}

Process::Process() {}

Process::~Process() {}

Long Process::currentId() { return ProcessImpl::currentId(); }

Process Process::launch(const String& command) { return launch(command, {}); }

Process Process::launch(const String& command, const Args& args) {
  return ProcessImpl::launch(command, args);
}

Long Process::id() const { return _impl->id(); }

Boolean Process::running() const { return _impl ? _impl->running() : false; }

Int Process::wait() const { return _impl->wait(); }

void Process::kill() { _impl->killImpl(); }
...
Process ProcessPool::create() {
  std::unique_lock<std::mutex> locker(_mutex);
  if (_freeNodes.empty()) _cond.wait(locker);
