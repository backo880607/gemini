...
HANDLE ProcessImpl::process() const
{
	return _hProcess;
}
...
std::shared_ptr<ProcessImpl> ProcessImpl::launch(const String & command, const Args & args)
{
	...
	if (rc) {
		...
		return std::shared_ptr<ProcessImpl>(new ProcessImpl(processInfo.hProcess, processInfo.dwProcessId));
	}
	else {
