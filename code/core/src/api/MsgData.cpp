...
uint8_t s_iv[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
MSG_UINT32 MsgData::getAESCBCSize() const {
  if (getEncryption() != Encryption::Encry_AES) return getDataSize();

  MSG_UINT32 remainders = getDataSize() % AES_BLOCK_LEN;
  return remainders == 0 ? getDataSize()
                         : (getDataSize() + (AES_BLOCK_LEN - remainders));
}
...
MsgData::mutable_buffer MsgData::prepare() {
  const std::size_t size =
      getAESCBCSize() - boost::asio::buffer_size(_data->data());
  boost::asio::streambuf::mutable_buffers_type buffer =
      _data->prepare(size > s_maxBlock ? s_maxBlock : size);
...
Boolean MsgData::format(const void* pData, std::size_t size) {
  ...
  if (cpt == CompressType::CPT_DEFLATE) {
    Int rt = compress((Bytef*)comBuf.begin(), (uLongf*)&postCompressSize,
                      (const Bytef*)pData, (uLong)size);
    if (rt != Z_OK) {
      return false;
    }
  ...
  if (encry == Encryption::Encry_AES) {
    ...
    // AES128_CBC_encrypt_buffer((uint8_t*)encryBuf.begin(), (uint8_t*)data,
    // (uint32_t)getHead().size, s_key, s_iv);
    _data->consume(_data->size());
...
Boolean MsgData::parse() {
  ...
  if (encry == Encryption::Encry_AES) {
    ...
    // AES128_CBC_decrypt_buffer((uint8_t*)decryBuf.begin(), (uint8_t*)data,
    // (uint32_t)getAESCBCSize(), s_key, s_iv);
    _data->consume(_data->size());
    _data->prepare(getHead().size);
...
void MsgData::reset() {
  _data = std::shared_ptr<boost::asio::basic_streambuf<std::allocator<Char>>>(
